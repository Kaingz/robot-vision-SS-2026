...
def inference_stereo(model,
    ...
    for i in range(num_samples):
        ...
        save_name_yml = save_name[:-4] + '_raw.yml'
        cv_file = cv2.FileStorage(save_name_yml, cv2.FILE_STORAGE_WRITE)
        cv_file.write("disparity", disp)
        cv_file.release()

        min_val = np.percentile(disp, 3)
        max_val = np.percentile(disp, 97)
        
        # Clip outliers
        disp = np.clip(disp, min_val, max_val)

        disp_normalized = cv2.normalize(disp, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        disp_gray = disp_normalized.astype(np.uint8)
        cv2.imwrite(save_name, disp_gray)
