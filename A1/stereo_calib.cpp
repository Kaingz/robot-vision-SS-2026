...
StereoCalib(const vector<string>& imagelist, Size inputBoardSize, string type, float squareSize, float markerSize, cv::aruco::PredefinedDictionaryType arucoDict, string arucoDictFile, bool displayCorners = false, bool useCalibrated=true, bool showRectified=true)
{
    ...
    double cm_left[3][3] = {
        { 1393.0218357625358, 0., 1037.8982633247501}, 
        {0., 1399.2372724532866, 649.99871810753609}, 
        {0., 0., 1.}
    };
    cameraMatrix[0] = Mat(3, 3, CV_64F, cm_left);

    double dc_left[5] = {
        -0.16265223527071923, 0.0020744390167396387, 0.00032001410185710736, 0.00051605179423790657, 0.016432959225571132
    };
    distCoeffs[0] = Mat(5,1, CV_64F, dc_left);

    double cm_right[3][3] = {
        {1380.5338155620468, 0., 1140.2069078456434}, 
        {0., 1387.1274157049104, 570.92245864295728}, 
        {0., 0., 1.}
    };
    cameraMatrix[1] = Mat(3, 3, CV_64F, cm_right);

    double dc_right[5] = {
        -0.16100811114883304, -0.0034192019870866357, 0.00023493172647895795, 1.7483764962178381e-05, 0.019282889127672294
    };
    distCoeffs[1] = Mat(5,1, CV_64F, dc_right);
